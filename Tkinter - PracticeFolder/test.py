...
class photoLabel(Label):
    def __init__(self,root,filepath,ratio):
        ...
        self = Label(root, image=new_img)
        self.image = new_img
        self.grid(row=1,column=0)

    def createimagelabel(self,filepath,ratio):
        # new_img = self.getimage(filepath,ratio)
    ...
    def labelchangeimage(self,root,filepath,ratio):
        self.grid_forget()
        new_img = getimage(filepath,ratio)
        self = Label(root, image=new_img)
        self.image = new_img
...
class MenuBar(Menu):
    def __init__(self,root):
        ...
        file.add_command(label="Open",command=lambda:canvas.canvaschangeimage(root,'imag/3F.jpg',0.1))  
